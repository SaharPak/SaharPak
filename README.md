<h1 align="center">Hi, I'm Sahar 👋</h1>

<p align="center">
  <strong>Community & Ecosystem Builder · Product Manager</strong> based in Espoo, Finland<br/>
  I build communities, and the products that serve them
</p>

<p align="center">
  <a href="https://medium.com/@saharlead"><img alt="Medium" src="https://img.shields.io/badge/Medium-12100E?style=flat&logo=medium&logoColor=white" /></a>
  <a href="https://www.youtube.com/@TechImmigrants"><img alt="YouTube" src="https://img.shields.io/badge/YouTube-FF0000?style=flat&logo=youtube&logoColor=white" /></a>
</p>

---

### About me

- 🌍 **Founder of Tech Immigrants**, a tech community I built from scratch to 53K+ followers across platforms, helping thousands of people land jobs and navigate careers in tech
- 🤝 **n8n Ambassador**, funded to run local meetups. My mission: grow the Finnish startup ecosystem into a stronger Nordic and global one
- 🚀 **Product Manager** at a unicorn B2B SaaS company (REST API & Identity), partnering across 10+ product teams and 500+ enterprise customers
- 💡 I run communities like products: I measure the real pain points, then build the events, tools, and content that move people to action
- 🔧 I build with **Cursor, Claude Code, n8n, Cloudflare** — bridging product thinking with hands-on building

---

### 🛠️ Featured Projects

| Project | Description | Stack |
|---------|------------|-------|
| 🌐 **Tech Immigrants Community** | 53K+ followers across [YouTube](https://www.youtube.com/@TechImmigrants), [Telegram channel](https://t.me/Tech_Immigrants), and [Telegram group](https://t.me/techimmigrants), built from zero | Community · Events · Content |
| 🎯 **[cvroast.dev](https://github.com/SaharPak/cvroast-dev)** | AI-powered CV scoring tool. Free, no signup, open source (MIT) | Claude API · Cloudflare Pages/Functions |
| 📝 **[TechImmigrants/cv-builder](https://github.com/TechImmigrants/cv-builder)** | Open-source CV builder, scaled with the community (36 contributors) | TypeScript · Open Source |
| 👥 **[productbuilders.app](https://productbuilders.app)** | A space where product builders show their work and get feedback in live demo sessions | Community · Product |

---

### 🔧 Stack & Tools

<p>
  <img src="https://img.shields.io/badge/Cursor-000000?style=flat&logo=cursor&logoColor=white" />
  <img src="https://img.shields.io/badge/Claude-D97757?style=flat&logo=anthropic&logoColor=white" />
  <img src="https://img.shields.io/badge/n8n-EA4B71?style=flat&logo=n8n&logoColor=white" />
  <img src="https://img.shields.io/badge/Docker-2496ED?style=flat&logo=docker&logoColor=white" />
  <img src="https://img.shields.io/badge/Cloudflare-F38020?style=flat&logo=cloudflare&logoColor=white" />
  <img src="https://img.shields.io/badge/TypeScript-3178C6?style=flat&logo=typescript&logoColor=white" />
</p>

---

### 🌱 What I'm working on

- Growing **Tech Immigrants** and bringing the community into more in-person events and meetups
- Expanding from local events toward the broader **Nordic and global startup ecosystem**
- Scaling **cv-builder** into a fully community-driven open-source product
- Exploring agentic AI patterns and shipping small builder tools for founders
